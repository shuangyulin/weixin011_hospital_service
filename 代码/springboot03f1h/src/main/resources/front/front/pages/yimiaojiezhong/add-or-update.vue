<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<input :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","borderColor":"rgba(215, 215, 215, 1)","borderRadius":"0px","color":"rgba(255, 144, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' :disabled="ro.jiezhongyuwei" v-model="ruleForm.jiezhongyuwei" placeholder="接种余位"></input>
			</view>
			...
			<!-- 否 -->
				...
				<textarea :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(215, 215, 215, 1)","borderRadius":"0px","color":"rgba(255, 144, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"280rpx"}' v-model="ruleForm.shuoming" placeholder="说明"></textarea>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`yimiaojiezhong`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																				if(this.ruleForm.jiezhongfeiyong&&(!this.$validate.isIntNumer(this.ruleForm.jiezhongfeiyong))){
					this.$utils.msg(`接种费用应输入整数`);
					return
				}
																																								if((!this.ruleForm.jiezhongyuwei)){
					this.$utils.msg(`接种余位不能为空`);
					return
				}
												if(this.ruleForm.jiezhongyuwei&&(!this.$validate.isIntNumer(this.ruleForm.jiezhongyuwei))){
					this.$utils.msg(`接种余位应输入整数`);
					return
				}
																																																																																																																																																																																																																								if(this.ruleForm.id){
					await this.$api.update(`yimiaojiezhong`, this.ruleForm);
				}else{
					await this.$api.add(`yimiaojiezhong`, this.ruleForm);
				}
